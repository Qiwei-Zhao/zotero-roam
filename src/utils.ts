...
function transformDOIs<T extends { doi: string | false | null, externalIds?: { DOI?: string } }>(arr: T[]) {
	return arr.map(elem => {
		const { doi, externalIds, ...rest } = elem;
		// Try to get DOI from externalIds first, then fall back to doi field
		const doiValue = externalIds?.DOI || doi || null;
		return {
			doi: parseDOI(doiValue),
			externalIds,
			...rest
		};
	});
